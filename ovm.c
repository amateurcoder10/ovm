...
struct symbol;
struct closure;
typedef struct object *(*method_t)(struct closure *closure, struct object *receiver, ...);//typedefining method_t as a pointer to a function with 2 args varargs and returns a pointer to an object
...
static struct closure *vt_bind(struct object *rcv,       struct symbol *selector);//selector is the message
...
{
	static int nesting = 0;//retains its value across calls of bind
	struct closure *c;
	struct vtable *rvt = vtof(rcv);//vtable of the receiver

	assert(rvt);
	struct vtable *super = rvt->parent;//vtable of the parent 

	if ((selector == s_vtlookup) && (rcv == (struct object *)Proto_vt))//if the passed message is lookup and receiver is proto vt
		return (struct closure *)vt_lookup(0, rvt, selector);//perform a lookup and return a closure
	nesting++;//else increment nesting
...
static struct object *vt_lookup(struct closure *cls, struct vtable *self, struct symbol *key)
{
	...
	for (i = 0;  i < self->tally;  ++i)
		if (key == (struct symbol *)(self->keys[i]))
			return self->values[i];//method found in the vtable return value which is a closure
	if (self->parent) {
		assert(self != self->parent);
		return send(self->parent, s_vtlookup, key);//else send the message lookup to parent
	}
...
static struct object *cls_alloc(method_t method, struct object *env)//allocating a closure:it has a method and environment
{
	struct closure *cls;
...
{
	struct object *key = (struct object *)selector;//key is the method name
	int i;
	...
	vt->values[vt->tally++] = cls_alloc(method, 0);//values holds a closure--env=0
	return key;
}
...
static struct symbol *atom(char *string)
{
	...
	symbol = sym_alloc(string);//else allocate
...
void init_ovm(void)
{
	...
	Symbol_vt   = vt_delegate(0, Object_vt, "Symbol");//both delegate to object
	Closure_vt  = vt_delegate(0, Object_vt, "Closure");

	s_vtlookup     = atom("lookup");//pointer to key
